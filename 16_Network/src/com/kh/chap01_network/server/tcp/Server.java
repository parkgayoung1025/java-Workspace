...
public class Server {
	 ...
	 * 7) 스트림을 통해 입력 및 출력
	 * 8) 통신 종료
	 *    - input Stream, OutputStream, Socket을 전부 닫아줘야 함(close())
	 */
	
	public void serverStart() {
		// 0) 포트 번호 지정
		//    - 0 ~ 1023 시스템 포트 번호
		//    - 1024 ~ 65535까지의 포트 번호를 사용하는 것을 추천
		int port = 30027;
		int clientCnt = 0;
		...
		try {
			...
			while (true) {
				// 3) 접속 요청이 오면 요청 수락(accept()) 후 해당 클라이언트에 대한 소켓 객체 생성
				// 4) 클라이언트의 정보를 저장
				...
				while (true) {
					...
					if (message == null || "exit".equals(message)) {
						...
						pw.flush(); // 현재 스트림에 있는 데이터를 강제로 내보내는 메서드. close() 메서드 호출되면 자동으로 flush 호출 된다.
					}
				}
				// 8) 통신 종료
				br.close();
