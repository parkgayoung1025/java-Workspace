...
public class ExceptionRun {
	...
	public static void main(String[] args) {
		/*
		 * 에러(오류) 종류
		 * - 시스템 에러 : 컴퓨터의 오작동으로 인해 발생하는 에러 => 소스코드로 해결 안 됨(심각한 에러)
		 ...
		 * - 논리 에러 : 소스코드상으로 문제없고 실행했을 때도 에러가 발생하지 않지만 의도한 대로 결과가 나오지 않는 경우
		 * 
		 * 시스템 에러를 제외한 컴파일 에러, 런타임 에러, 노리 에러와 같은 비교적 덜 심각한 것들을 가지고 작업을 함
		 * 이런 것들을 "예외"라고 부름(Exception)
		 * 
		 * 예외 처리를 해야 하는 이유 : 예외 발생 시 프로그램이 비정상적으로 종료되는 것을 막기 위함.
		 * 
		 * 예외 처리 방법
		 * 1. try ~ catch (직접처리)
		 * 2. throws 이용 (떠넘기기)
