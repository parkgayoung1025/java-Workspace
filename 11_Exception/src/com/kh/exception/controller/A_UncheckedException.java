...
public class A_UncheckedException {
	 ...
	 * - ...
	 * 
	 * => 이러한 RuntimeException과 관련한 예외는 충분히 예상 가능한 상황이기 때문에 예외 자체가 발생이 안되게끔 조건문으로 해결이 가능함 
	...
	public void method1() {
		...
		int num2 = sc.nextInt();
		// 해결방법 1. 조건문으로 처리(애초에 예외 자체가 발생되지 않게끔 if문으로 조건 검사하기) => 예외처리x
//		if (num2 == 0) {
...
//		System.out.println("프로그램 종료");
		...
		try {
			System.out.println("나눗셈 연산 결과 : " + (num1 / num2)); // ArithmeticExceptoin
...
//		if(size > 0) { // 방법 1. if문으로 size값이 양수값일 때만 생성하도록 제어
//			int [] arr = new int[Math.abs(size)]; // 방법 2. 항상 양의 정수값이 들어갈 수 있도록 abs메서드 사용
//		}
		
		// 이 코드들이 아래 코드보다 좀 더 잘 작성한 코드
//		try {
...
//		} // 다중 catch블럭이 줄어들었지만 정확히 어떤 에러인지 알 수 없음
	 ...
	 * 
	 * RuntimeException 계열은 충분히 예측 가능한 상황이기 때문에 조건문으로 해결하는 것을 권자
	 * 
	 * UncheckedException -> 필수로 체크하지는 안하도 되는 예외라는 뜻
	 */
