...
public class ListRun {
	...
	public static void main(String[] args) {
		 ...
		 * 
		 * 정리해 보면 데이터들이 새로이 추가되거나, 삭제가 되거나, 수정이 되는 기능(알고리즘)들이 이미 정의되어 있는 틀이 있다.
		 * == 컬렉션
		 ...
		 *    => 한 번 지정된 크기는 변경이 불가
		 *       새로운 값을 추가하고자 할 때 크기가 오버될 경우 새로운 크기의 배열을 만들고 기존의 내용들을 복사해주는 코드를 직접 구현해야함.
		 * 2. 배열 중간 위치에 새로운 데이터를 추가하거나 삭제해야 하는 경우 기존의 값들을 땡겨주는 코드를 직접 짜야함.
		 * 3. 한 타입의 데이터들만 저장 가능
		 * 
		 * - 컬렉션의 장점
		 * 1. 크기에 제약이 없다.
		 *    => 크기지정을 해줄 필요도 없고 만일 크기 지정을 해놔도 알아서 크기가 늘어나면서 새로운 데이터를 추가해주는 코드가 이미 정의되어 있음
		 * 2. 중간에 값을 추가하거나 삭제하는 경우 값을 땡겨주는 코드가 이미 메서드로 정의되어 있음.
		 ...
		 * 컬렉션의 3가지 분류
		 * 
		 * List 계열 => 담고자 하는 값(value)만 저장 / 값 저장 시 순서 유지 됨(index의 개념이 있음) / 중복값 허용
		 *             ex) ArrayList, Vector, LinkedList => ArrayList가 제일 많이 쓰임(90% 이상)
		 * 
		 * Set 계열 => 담고자 하는 값(value)만 저장 / 값 저장 시 순서 유지 X(index 개념이 없음) / 중복값 허용 X
		 *            ex) HashSet, TreeSet
		 *            
		 * Map 계열 => 키(key) + 담고자하는 값(value) 세트로 저장 / 값 저장 시 순서 유지 X(index 개념 없음) / key 중복 허용 X
		...
		/*
		 * 제네릭(<E --> Element>)
		 * 별도의 제네릭 제시 없이 컬렉션 객체를 생성하게 되면 해당 컬렉션에 다양한 타입의 데이터 값들이 담길 수 있음(Default E == Object)
		 * 
		 * 별도의 제네릭 설정으로 <Music>으로 하게 되면 (E == Music)
		 * 
		 * 제네릭을 설정 하는 이유
		 * 1. 명시한 타입의 객체만 저장 가능 하도록 타입의 제한을 두기 위해서
...
//		list.add(new Music("hype boy", "new jeans"));
//		list.add(new Music("die for you", "the weekends"));
//		list.add("흐흐"); Music 객체(title, artist)만 들어가게 설정해놨기 때문에 다른 건 안 됨 
		// ㄴ> ArrayList<Music> list => ArrayList list 으로 하면 흐흐 출력 가능
		/*
		 * 순서가 유지 되면서 값 추가 됨
		 * 각 index에 담겨있음
		 * 크기에 제약이 없음
		...
		// 중간에 값 추가 시 알아서 기존의 값들을 뒤로 땡겨주는 작업이 내부적으로 진행 됨
		System.out.println(list);
		
		// 3. set(int index , E e) : 리스트에 원하는 위치의 인덱스 값을 제시하면 전달하는 e값으로 변경해주는 메서드
		list.set(0, new Music("p;ㅠ", "BTS"));
...
//		System.out.println(m.getTitle());
		...
		// 7. subList(int index1, int index2) : 해당 리스트로부터 index1에서 index2까지의 데이터 값들을 추출해서 새로운 List로 반환시켜주는 메서드
