...
public class B_ArrayCopy {
	...
	public void method1() {
		...
		//이유는 얕은 복사가 이루어졌기 때문 => 배열의 주소값이 복사되어 원본과 복사본은 사실상 동일한 데이터를 공유하고 있다
...
//		System.arraycopy(origin, 2, copy, 9, 2);// 인덱스의 범위를 벗어난 복사는 불가능
	...
	public void method4() {
		...
		int [] copy = Arrays.copyOf(origin, 3); // copy 배열의 크기 == 복사할 개수
		
		for(int i=0; i<copy.length; i++) {
			System.out.print(copy[i]+" ");
		}
		/*
		 * 2. System.arraycopy 메서드 : 몇번 인덱스부터 몇 개를 어느 위치의 인덱스에 복사할 것인지 세밀하게 지정 가능
		 * 
		 * 3. Arrays.copyOf 메서드 : 무조건 원본 배열의 0번 인덱스부터 내가 제시한 개수만큼 복사 진행됨
		 *                          내가 제시한 길이가 원본 배열도 크다면 나머지에는 0의 값으로 채워서 복사해줌
		 */
	}
