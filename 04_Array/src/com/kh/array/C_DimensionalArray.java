...
public class C_DimensionalArray {
	...
	public void method1() {
		 ...
		 */
		for(int i=0; i<arr.length; i++) { //i<=arr.length-1; 가능
			for(int j=0; j<arr[i].length; j++) { //j<=arr[i].length-1; 가능
...
//				System.out.print(arr[i][j]+" ");
	...
	public void method4() {
		/*
		 * 가변 배열
		 * 행의 크기는 정해져 있으나 각가의 행별로 열의 개수가 정해져 있지 않은 상태
		 * 이차원 배열 == 일차원 배열을 여러 개 묶은 형태
		 * 즉, 묶여있는 일차원 배열의 길이가 꼭 같을 필요는 없다.
		 * 
		 * 단, 행 크기는 생략이 불가, 반드시 지정해야 함
		...
		//System.out.println(arr[0][2]); //오류
		// 아직 각 행별로 열의 크기가 지정되지 않았으므로 null 값이 대입된 상태에서 특정 인덱스로  접근하려 했기 때문에
	...
	public void method7() {
		// int형 2차원 배열을 만들어
		// 0번행에는 국어 점수를 3개 입력받고
		// 1번행에는 영어 점수를 3개 입력받은 후
		// 각각 반복문을 활용해서 출력
		// xx 점수를 입력하세요 :
		// xx 점수 : xx xx xx
