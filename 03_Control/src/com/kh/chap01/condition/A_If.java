...
public class A_If {
	 ...
	 * 
	 * 조건문
	 * 조건식을 통해 결과 값이 참이냐 거짓이냐를 판단해서 그에 해당하는 코드를 실행
	 * 
	 * 조건식
...
//	삼항 연산자를 조건문으로 바꿔보기
	...
	public void method3() {
		 ...
		 * } else {
		 *     위의 모든 조건식의 결과가 false라면 실행 할 코드
		 * }
		 * 
		 */
		if(0 < num) {
	...
	public void method5() {
		...
		if(gender == 'm' || gender == 'M') {
			...
			//만약에 이 아래 코드를 실행시키고 싶지 않다면 return 사용
			return;// 해당 일반메서드를 빠져나감(메서드를 호출했던 곳으로 돌아감)
		}
		//ㅇㅇㅇ님은 xxx이다.
		System.out.printf("%s님은 %s이다. \n", name, result);
		//->실행시 오류남. 위에 String result = "";로 초기화 한 번 해두는게 필수
	}
	
	public void method6() {
//		특이케이스. 문자열 간의 동등 비교 (문자열 == 문자열)
		//사용자에게 이름을 입력 받아 박가영과 일치하는지 비교하기
		...
		//String은 참조자료형
		if(name == "박가영") {
			System.out.println("박가영님 반갑습니다.");
		} else {
			System.out.println("박가영님이 아니신가보네요?");
		}
		/*
		 * 기본자료형 : boolean, byte, short, int, long, float, double, char
		 * 참조자료형 : String // 특이함
		 * 
		 * 기본자료형간 비교시 == != 사용이 가능하지만, 참조자료형은 동등비교 연산시 정상적으로 연산이 안됨
		 * 그럼 어떻게 하나? => 문자열.equals() 방법으로 비교를 함
		 */
		//기본자료형 : charAt()은 , 참조자료형 : equals()
		if(name.equals("박가영")) {//즉, 반환값이 true or false로 리턴해줌
			System.out.println("박가영님 반갑습니다.");
		} else {
			System.out.println("박가영님이 아니신가보네요?");
		}
